...
sed -e '/^#/d' ipsum.txt > nocomments.txt && echo -e "\e[2mComments Removed\e[0m"

echo -e "Removing \e[1mLines That End With 10\e[0m"
sed -i '/10$/d' nocomments.txt && echo -e "\e[2mLines Containing 10s At The End Removed\e[0m"

echo -e "Removing \e[1mUnnecessary Numbers\e[0m"
sed 's/.$//' nocomments.txt > removed_digit_at_end.txt && echo -e "\e[2mCleaned Unnecessary Lines\e[0m"

echo -e "Removing \e[1mSpaces & Tabs\e[0m"
sed 's/[[:blank:]]*$//' removed_digit_at_end.txt > removed_tabs_and_spaces.txt && echo -e "\e[2mCleaned Spaces & Tabs\e[0m"

echo -e "Inserting \e[1mPrefix\e[0m"
sed -e 's/^/<fpc4:Str dt:dt="string">http:\/\//' removed_tabs_and_spaces.txt > inserted_prefix.txt && echo -e "\e[2mDone Inserting Prefix\e[0m"

echo -e "Inserting \e[1mSuffix\e[0m"
awk '{print $0"</fpc4:Str>"}' inserted_prefix.txt > ip-blacklist.xml && echo -e "\e[35mDone Inserting Suffix\e[0m"

echo -e "Adding \e[1mProper Tabs\e[0m"
sed -i -e 's/^/\t\t\t\t\t\t/' ip-blacklist.xml && echo -e "\e[2mAdded Proper Tabs\e[0m"
